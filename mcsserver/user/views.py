...
from mcs.common_code import CommonExportListMixin, CommonBatchDestroyView, get_cur_sheet, get_sheet_data
...
class LoginView(ObtainJSONWebToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if not permissions and not user.is_superuser:
...
class GroupPermissions(APIView):
    ...
    def get(self, request):
        ret = []
        parent_permissions = Permissions.objects.filter(parent__isnull=True).order_by('category_name', 'id')
