  ...
  <div>
    ...
    <div v-loading="loading" class="center-box">
      ...
      >
        ...
        />
        <el-table-column
          prop="back_process_name"
          label="下游工序名称"
          min-width="20"
        />
        <el-table-column
          prop="previous_process_name"
          label="上游工序名称"
          min-width="20"
        />
        <el-table-column
    ...
    >
      <el-form ref="ruleForm" :model="currentObj" :rules="rules" label-width="150px" inline>
        <el-form-item label="下游工序名称" prop="back_process">
          <el-select
            v-model="currentObj.back_process"
            size="small"
            placeholder="请选择"
          >
            <el-option
        ...
        <el-form-item label="上游工序名称" prop="previous_process">
          <el-select
            v-model="currentObj.previous_process"
            clearable
            size="small"
            placeholder="请选择"
          >
        ...
        <el-form-item label="库存上限" prop="stock_limit">
          <el-input-number v-model="currentObj.stock_limit" size="small" />
        </el-form-item><br>
...
import { receiveAlarm, receiveAlarmUpdate, receiveAlarmDel } from '@/api/jqy'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        previous_process: [
          { required: true, message: '请选择上游工序', trigger: 'change' }
        ],
        back_process: [
          { required: true, message: '请选择下游工序', trigger: 'change' }
        ],
        stock_limit: [
  ...
  methods: {
    ...
    async getList() {
      try {
        this.loading = true
        const data = await receiveAlarm('get', null, { params: this.getParams })
    ...
    deleteFun() {
      ...
      this.$confirm('此操作删除不可逆, 是否继续?', '提示', {
        ...
        receiveAlarmDel('post', null, { data: { obj_ids: arr }})
          .then((response) => {
            this.$message({
              type: 'success',
              message: '操作成功!'
    ...
    useFun(val, bool) {
      ...
      this.$confirm(`此操作${val}, 是否继续?`, '提示', {
        ...
        receiveAlarmUpdate('post', null, { data: { 'obj_ids': arr }})
          .then((response) => {
            this.$message({
              type: 'success',
              message: '操作成功!'
      ...
      this.$refs.ruleForm.validate(async(valid) => {
        if (valid) {
          try {
            ...
            await receiveAlarm(_api, this.currentObj.id || null, { data: this.currentObj })
