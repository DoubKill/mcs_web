    ...
    <div class="top-search-box">
      <el-form :inline="true">
        ...
        <el-form-item label="AGV编号">
          <el-input v-model="getParams.agv_no" size="small" clearable placeholder="AGV编号" />
        </el-form-item>
        <!-- <el-form-item label="任务当前状态">
          <el-select
            v-model="getParams.task_state"
            clearable
            size="small"
          >
            <el-option
...
<script>
import { agvTasks } from '@/api/base_w'
...
export default {
  ...
  methods: {
    async getList() {
      try {
        this.loading = true
        const data = await agvTasks('get', null, { params: this.getParams })
    ...
    changeDate(date) {
      this.getParams.start_position = date ? date[0] : ''
      this.getParams.end_position = date ? date[1] : ''
      this.changeList()
    },
      ...
      this.$refs.ruleForm.validate(async(valid) => {
        if (valid) {
          try {
            ...
            await agvTasks(_api, this.currentObj.id || null, { data: this.currentObj })
